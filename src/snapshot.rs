...
/// Get outputs and treasury_output_amount from a snapshot
pub fn get_snapshot_outputs_and_treasury(path: &str) -> Result<(Vec<OutputData>, u64)> {
    ...
    if let SnapshotKind::Full = header.kind() {
        ...
        Ok((outputs, full_header.treasury_output_amount()))
    } else {
